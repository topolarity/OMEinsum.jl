...
OMEinsum._reshape(x::CuArray, sz) = reshape(x, sz)
OMEinsum._reshape(x::Adjoint{T, <:CuArray{T}} where T, sz) = reshape(CuArray(x), sz)
OMEinsum._reshape(x::Transpose{T, <:CuArray{T}} where T, sz) = reshape(CuArray(x), sz)
