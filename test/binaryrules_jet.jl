...
@testset "analyse binary + JET" begin
    size_dict = Dict(1=>1, 2=>2, 3=>3, 4=>4, 6=>6, 7=>7, 8=>8)
    @test_opt OMEinsum.analyze_binary(Val((1,2,3,4,8)), Val((2,6,6,8,4,2)), Val((7,2,1,2,2,6)), size_dict)
end

@testset "binary rules + JET" begin
        ...
        for i1 in [(), ('i',), ('j',), ('i','j'), ('j', 'i')]
            for i2 in [(), ('k',), ('j',), ('k','j'), ('j', 'k')]
                for i3 in [(), ('i',), ('k',), ('i','k'), ('k', 'i')]
                    ...
                    code = StaticEinCode((i1_,i2_),i3_)
                    ...
                    if rule isa SimpleBinaryRule
                        ...
                        @test_opt binary_einsum!(rule, a, b, out, true, false)
                    else
                        @test_opt einsum(code, (a, b), size_dict)
                    end
                end
            end
        end
...
@testset "match binary rules + JET" begin
    for code in [ein",->", ein",k->k", ein"i,->i", ein"j,j->", ein"i,k->ik", ein"i,k->ki",
        ...
        @test_opt OMEinsum.match_rule(code)
    end
end

@testset "binary einsum + JET" begin
    for code in [
        ein",->",
        ein"ijl,jl->il",
        ein"ab,bc->ac",
        ...
        @test_opt einsum(code, (xs...,), size_dict)
    end
end

@testset "regression test + JET" begin
    x, y = randn(fill(2, 8)...), randn(fill(2, 5)...)
    code = StaticEinCode(((10, 5, 4, 2, 6, 6, 7, 9), (1, 9, 8, 2, 10)), (4, 10, 9, 7, 10))
    @test_opt code(x, y)
    @test_opt loop_einsum(code, (x, y), OMEinsum.get_size_dict(OMEinsum.getixs(code), (x, y)))
end

@testset "polynomial scalar mul + JET" begin
    @test_opt ein",->"(OMEinsum.asarray(Polynomial([1.0])), OMEinsum.asarray(Polynomial([1.0])))
end
